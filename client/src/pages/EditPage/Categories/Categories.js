...
import { getFullViewPromise } from '../../../utils/dataHandler.js';
 ...
 */
function Categories({ data, index, handleChange, val }) {
  ...
  useEffect(() => {
    ...
    let currId = currUrl[currUrl.indexOf("edit-artefact") + 1];
    console.log("Id:", currId);
    let startLabel = "";
    getFullViewPromise(currId)
      .then((response) => {
        if(data.name === "category"){
          console.log("Reached for category")
          console.log(response.data)
          startLabel = response.data.result.category.category_name
          setLabel(startLabel);
          
        }else{
          startLabel = response.data.result.associated.person
          setLabel(startLabel);
        }
